...
describe("Shallow QuizSetup", () => {
  ...
  it("Category dropdown should contain 5 (for now) topic options in <option /> tags", () => {
    ...
    expect(
      wrapper
        ...
        .at(0)
        .props()
    ).toHaveProperty("value", "9"); // General Knowledge
    expect(
      wrapper
        .find("select")
        .at(1)
        ...
        .at(1)
        .props()
    ).toHaveProperty("value", "21"); // Sports
    expect(
      wrapper
        .find("select")
        .at(1)
        ...
        .at(2)
        .props()
    ).toHaveProperty("value", "17"); // Science and Nature
    expect(
      wrapper
        .find("select")
        .at(1)
        ...
        .at(3)
        .props()
    ).toHaveProperty("value", "23"); // History
    expect(
      wrapper
        .find("select")
        .at(1)
        ...
        .at(4)
        .props()
    ).toHaveProperty("value", "28"); // Vehicles
    ...
    expect(
      wrapper
        ...
        .at(0)
        .text()
    ).toEqual("General Knowledge"); // General Knowledge
    expect(
      wrapper
        .find("select")
        .at(1)
    ...
    expect(
      wrapper
        ...
        .at(2)
        .text()
    ).toEqual("Science & Nature"); // Science and Nature
    expect(
      wrapper
        .find("select")
        .at(1)
  ...
  it("Link should link to Quiz Page", () => {
    expect(
      wrapper
        .find("Link")
        .at(0)
...
describe("mounted quizSetup", () => {
  let onChange = jest.fn(); // For testing onChange functions are called when intended
  let routerWrapper = mount(
    <MemoryRouter>
      <QuizSetup handleChange={onChange} />
    </MemoryRouter>
  );
  let wrapper = routerWrapper.find(QuizSetup).at(0);

  it("First dropdown should have an onChange that is called when an option is selected", () => {
    wrapper
      .find("select")
      .at(0)
      .simulate("change", { target: { value: "medium" } });
